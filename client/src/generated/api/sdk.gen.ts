 ...
 */
export const getChatMessage = <ThrowOnError extends boolean = false>(options: Options<GetChatMessageData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetChatMessageResponse, GetChatMessageError, ThrowOnError>({
        url: '/v1/chat/{userID}',
        ...options
    });
};
...
/**
 * Get Chat List
 * Get chat list. From all chats with the user id, poll chat to wait for new message sent by users connected with the user id. When receiving messages from the users or time out, polling again
 */
export const getChatList = <ThrowOnError extends boolean = false>(options?: Options<GetChatListData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetChatListResponse, GetChatListError, ThrowOnError>({
        url: '/v1/chatlist',
        ...options
    });
};
