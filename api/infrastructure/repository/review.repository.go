...
func (r *reviewRepositoryImpl) FindReviewsWithTruncatedBuyerBySellerID(ctx context.Context, sellerID int64) ([]*domain.ReviewWithTruncatedBuyer, error) {
	...
	err := r.db.Model(&domain.Review{}).
		Select("*, CASE WHEN LENGTH(users.first_name) > 3 THEN CONCAT(SUBSTRING(users.first_name, 1, 3), '***') ELSE users.first_name END AS buyer_truncated_first_name, CASE WHEN LENGTH(users.last_name) > 3 THEN CONCAT(SUBSTRING(users.last_name, 1, 3), '***') ELSE users.last_name END AS buyer_truncated_last_name, users.photo AS buyer_photo").
